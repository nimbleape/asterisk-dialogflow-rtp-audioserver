...
class DialogFlowConnector extends EventEmitter {
  ...
  _createNewDialogFlowStream() {
    ...
    this._dialogFlowStream.write(this._initialStreamRequest);
    ...
    for (let i = 0; i < frames+1; i++) {
      setTimeout(() => {
        ...
        if (!rtp) {
          ...
          rtp.type = 11;
          rtp.source = 1234234243;
        } else {
          rtp.payload = buf;
          rtp.time += buf.length / 2;
          rtp.seq++;
        }
        this._asteriskAudioStream.outWStream.write(rtp.packet);
        pos = newpos
      }, i * 20)
    }
  }
