...
class DialogFlowConnector extends EventEmitter {
  ...
  _createNewDialogFlowStream() {
    if (this._closed) {
      this.log.info(
        'Requested a new Dialogflow even though we closed the connector'
      );
      return;
    }
    ...
    this._dialogFlowStream = this._dialogFlowClient
      ...
      .on('data', (data) => {
        ...
        if (data.detectIntentResponse) {
          ...
          if (
            !this._enableDialogflowOutputSpeech &&
    ...
    if (this._numOfStreamCycles === 1 && this._initialEventName) {
      ...
      tmpInitialStreamRequest = JSON.parse(
        JSON.stringify(this._initialStreamRequest)
      );
    ...
    if (this._dialogFlowStream && this._dialogFlowStream.writable) {
      try {
        this._dialogFlowStream.write(
          tmpInitialStreamRequest || this._initialStreamRequest
        );
      } catch (err) {
  ...
  _sendAudioToAsterisk(dialogFlowData) {
    ...
    let config =
      dialogFlowData.detectIntentResponse.outputAudioConfig ||
      dialogFlowData.replyAudio.config;
    let audio =
      dialogFlowData.detectIntentResponse.outputAudio ||
      dialogFlowData.replyAudio.audio;
  ...
  _sendDataToApp(dialogFlowData) {
    ...
    if (dialogFlowData.recognitionResult) {
      this.log.info(
        { transcript: dialogFlowData.recognitionResult.transcript },
        'Intermediate transcript'
      );
      body.transcript = dialogFlowData.recognitionResult;
    } else {
      this.log.info(
        { intent: dialogFlowData.detectIntentResponse.queryResult },
        'Detected intent'
      );
  ...
  close() {
    this.log.info(
      'Asterisk Stream closed so closing connection to DialogFlow and doing tidy up'
    );
    this._closed = true;
    clearTimeout(this._timeoutRef);
